...
public class CapabilityService {
	...
	public CapabilityService(UUID effectiveModelId, int effectiveModelVersion) {
		super();
	...
	public Capability getCapabilityById(String id, int version) {
...
//	}
	...
	public List<Capability> getCapabilityByProperty(List<Properties> propInput, String modelId, int modelVer){
		List<Capability> retC = new ArrayList<Capability>();
		
		//TODO: get props parent that are capability - POST
		HttpEntity<List<Properties>> requestBody = new HttpEntity<>(propInput);
		Capability [] foundCapability = restTemplate.postForObject(url + "/element/class/Capability", requestBody, Capability[].class);
		//TODO: filter parents as per the model
		
		for (int i=0; i<foundCapability.length;i++) {
			
			ElementsInModel eim =eimRepository.findByElementIdAndElementVersionAndModelIdAndModelVersion(foundCapability[i].getId().toString(), foundCapability[i].getVersion(), modelId, modelVer);

			if(eim!=null) {
				retC.add(foundCapability[i]);
			}
